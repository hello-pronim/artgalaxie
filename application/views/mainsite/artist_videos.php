...
<title>Art Galaxie – Artist Videos</title>
...
<div class="gallery-page-in ">
  ...
  <div class="section middle-menu-section-bg bg-none" style="padding:0;">
  <div class="middle-menu-section artist-information-menu shadow-none bg-none" data-aos="fade-up" data-aos-duration="2500"   data-aos-once="true" style="padding:0;">
    <div class="container">
      ...
      <div class="container"  data-aos="fade-down">
          <div class="style-page-in">
              <ul id="filter-list" class="nav nav-tabs artistsect-tabs text-center">
                <?php $i=1; $paginate_gcid = '';
               // echo "<pre>"; print_r($galleries); echo "</pre>";
                foreach($galleries as $galleriesRs){ ?>
                
                    <?php 
                            $uri = parse_url($_SERVER["REQUEST_URI"], PHP_URL_PATH);
						    if($gcid == $galleriesRs['cat_id']) {
                                $class_act = 'class="active"';
                                $paginate_gcid = '?gcid='.$galleriesRs['cat_id'];
                            } else {
                                $class_act = '';
                                //$paginate_gcid = '';
                            }
                        ?>
                         <li class="tab<?=$i?>">
							<a href="<?=base_url()?>artists-video/0.html?gcid=<?=$galleriesRs['cat_id']?>" <?=$class_act;?>><?=ucfirst(stripslashes($galleriesRs['cat_name']))?></a>
                        </li> 
 ...
 <?php if(!empty($artistVideos)){
  ...
  //echo "<br>GCID is ".$gcid;
  //echo "<pre>"; print_r($artistVideos); echo "</pre>";
  foreach($artistVideos as $artistVideosRs)
  { 
    
    //echo "<pre>";
    //print_r($artistVideosRs);
    ...
    $video = $video2 = $videoLib->getArtistVideoListNew('videos_link,id',$where); 
  ...
  ?>
    <div class="item video-sort  <?=$artistVideosRs['galleries_id']?>">
        ...
        <div class="slider artist-profile-slider artist-profile-video-slider">
              ...
              <?php  
                ...
                ?>
                  <li>  
                    ...
                    {
                    ?>
                        <iframe id="ytplayer" type="text/html" width="100%" height="650" src="<?=$videosRs['videos_link']?>>" frameborder="0"></iframe>
                    <?php
                    }
                    else
                    {
          ...
          <div id="carousel<?=$coursorID?>" class="flexslider flexslider-thumb">
            <ul class="slides">
               <?php 
               foreach($video2 as $videosRs2)
               {  
                    $video_arr = $videoLib->parseVideos($videosRs2['videos_link']);      
                    ?>
                    <li>
                        <span class="flexslider-thumb-img"> 
                        <img src="<?=$video_arr[0]['thumbnail'];?>" alt=""></span> 
                    </li>
               <?php  
               } 
               ?>
    ...
    <div class="middle-tab-section-bg blog-sect-heding sigl-blog-btn-block">
      <div class="container aos-init aos-animate" data-aos="fade-up" data-aos-once="true">
        ...
        <?php  if($previous>=0){ ?> 
        <a href="<?=site_url('artists-video/'.$previous.$paginate_gcid)?>" class="blog-btn-style">Previous</a>
        <?php } ?>
        
       <?php  if($next_start<$totalVideoCount){ ?> 
        <a class="blog-btn-style" href="<?=site_url('artists-video/'.$next_start.$paginate_gcid)?>">Next</a>
        <?php } ?>
        </div>
      </div>
    </div>
