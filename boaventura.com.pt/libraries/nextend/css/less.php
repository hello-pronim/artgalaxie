...
class nlessc {
	...
	protected function injectVariables($args) {
		$this->pushEnv();
		$parser = new nlessc_parser($this, __METHOD__);
	...
	protected function makeParser($name) {
		$parser = new nlessc_parser($this, $name);
	...
	protected function newFormatter() {
		$className = "nlessc_formatter_lessjs";
...
class nlessc_parser {
	...
	public function __construct($lessc, $sourceName = null) {
		...
		if (!self::$operatorString) {
			self::$operatorString =
				'('.implode('|', array_map(array('nlessc', 'preg_quote'),
					array_keys(self::$precedence))).')';

			$commentSingle = nlessc::preg_quote(self::$commentSingle);
			$commentMultiLeft = nlessc::preg_quote(self::$commentMultiLeft);
			$commentMultiRight = nlessc::preg_quote(self::$commentMultiRight);
	...
	protected function isDirective($dirname, $directives) {
		...
		$pattern = implode("|",
			array_map(array("nlessc", "preg_quote"), $directives));
	...
	protected function expressionList(&$exps) {
		...
		$exps = nlessc::compressList($values, ' ');
		return true;
	}
	...
	public function propertyValue(&$value, $keyName = null) {
		...
		$value = nlessc::compressList($values, ', ');
		return true;
	}
	...
	protected function openString($end, &$out, $nestingOpen=null, $rejectStrs = null) {
		...
		$stop = array("'", '"', "@{", $end);
		$stop = array_map(array("nlessc", "preg_quote"), $stop);
	...
	protected function literal($what, $eatWhitespace = null) {
		...
		if (!isset(self::$literalCache[$what])) {
			self::$literalCache[$what] = nlessc::preg_quote($what);
		}
	...
	protected function to($what, &$out, $until = false, $allowNewline = false) {
		...
		if (!$this->match('('.$validChars.'*?)'.nlessc::preg_quote($what), $m, !$until)) return false;
...
class nlessc_formatter_classic {
	public $indentChar = "  ";
...
class nlessc_formatter_compressed extends nlessc_formatter_classic {
...
class nlessc_formatter_lessjs extends nlessc_formatter_classic {
	public $disableSingle = true;
