...
;(function($,window,document,undefined) {
  ...
  var ZBox = {
    ...
    bi: function()
    {
      ...
      var iw = img.width;
      var ih = img.height;
      var ww = ZBox.window.width() - ZBox.o.margin * 8;
      var wh = ZBox.window.height() - ZBox.o.margin * 8;
      var cw = ww - 80;
      var ch = wh - 20;
      var ca = cw/ch;
      var ia = iw/ih;
      ...
      ZBox.im.width( nw );
      ZBox.im.height( nh );
      ZBox.c.css('left', Math.ceil( ZBox.o.margin + (ww-(nw+80)) / 1.3 ) + 'px' );
      ZBox.c.css( 'top', Math.ceil( ZBox.o.margin + (wh-(nh+20)) / 1.3 ) + 'px' );
      if( ZBox.i.length > 1 )
      {
//        if( ZBox.cin > 0 )
        {
          $('<div class="zbox-button left"><i class="fa fa-angle-left" aria-hidden="true"></i></div>').css('top',Math.ceil(((nh+20)/2)-6)+'px').appendTo(ZBox.c).click( ZBox.p );
        }
//        if( ZBox.cin+1 < ZBox.i.length )
        {
          $('<div class="zbox-button right"><i class="fa fa-angle-right" aria-hidden="true"></i></div>').css('top',Math.ceil(((nh+20)/2)-6)+'px').appendTo(ZBox.c).click( ZBox.n );
        }
      }
      $('<div class="zbox-button close"><i class="fa fa-times-circle-o" aria-hidden="true"></i></div>').appendTo(ZBox.c).click( ZBox.sdn );
      $('<div class="zbox-title">'+title+'</div>').appendTo(ZBox.c);
