...
class N2SmartsliderBackendSliderController extends N2SmartSliderController {
    ...
    public function actionPublishHTML() {
        if ($this->validateToken() && $this->validatePermission('smartslider_edit')) {
            N2Loader::import('libraries.export', 'smartslider');
            $export = new N2SmartSliderExport($this->sliderId);
            $files  = $export->createHTML(false);
            $path   = N2Filesystem::getBasePath() . '/sliders/' . $this->sliderId . '';


            foreach ($files AS $p => $content) {
                $folder = $path . '/' . $p;
                if (!N2Filesystem::existsFolder(dirname($folder))) {
                    N2Filesystem::createFolder(dirname($folder));
                }
                N2Filesystem::createFile($folder, $content);
            }
            header('LOCATION: ' . N2Uri::pathToUri($path . '/index.html'), TRUE, 301);
            exit;
        }
